...
const ContactIcon = (
  <svg
    className="w-10 h-10 text-[#8E9B72]"
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
    strokeWidth="1.5"
...
const ServiceIcon = (
  <Briefcase className="w-10 h-10 text-amber-500" strokeWidth="1.5" />
);
...
export default function MobileHomepage({ onLogout }) {
  ...
  const ActiveComponent = activeApp
    ? allApps.find((a) => a.id === activeApp)?.component
    : null;

  return (
    <div
      className="w-full h-[100dvh] overflow-hidden relative bg-no-repeat bg-center text-white bg-[#0a0a0a]"
      style={{
        backgroundImage: `url('/images/balidan.svg')`,
        backgroundSize: "70%",
      }}
    >
      {/* iOS Status Bar */}
